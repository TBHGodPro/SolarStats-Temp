...
export default function (playerData: PlayerData, mode: Mode): { string: string; stats: { [key: string]: unknown } } {
  let string = `${playerData.formattedNickname} §7- `;
  let stats = playerData.stats.duels;

  if (GameStatNames[mode]) {
    if (typeof GameStatNames[mode] === 'string') stats = stats[GameStatNames[mode]];
    else for (const item of GameStatNames[mode]) stats = stats[item];
